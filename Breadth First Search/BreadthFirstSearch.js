...
function addNeighborsToFrontier(board) {
	...
	for (var i = 0; i < neighborStates.length; i++) {
		if (inExplored(neighborStates[i])) continue;         // Makes graph search instead of
		if (isIdentical(board, neighborStates[i])) continue; // tree search
...
function printSolution(node) {
	...
	var n = node;
	while (n.prevBoard !== null) {    // Complete when previous board is null, i.e., initial state
...
function breadthFirstSearch(problem) {
	...
	frontier.enqueue(makeNode(problem, null));
